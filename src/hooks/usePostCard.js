...
export const usePostCard = (post, activeView = "home") => {
  ...
  const isCurrentUserPost = (() => {
    ...
    const userIds = [user.id, user.employee_id, user.user_id]
      .filter(Boolean)
      .map((id) => String(id));
    const authorIds = [
  ...
  const normalizePost = (rawPost) => {
    ...
    if (
      rawPost.isOptimistic &&
      rawPost.images &&
      Array.isArray(rawPost.images) &&
      rawPost.images.length > 0 &&
      rawPost.images[0].url
    ) {
    ...
    if (
      ...
      const countsObject =
        rawPost.reaction_counts.counts || rawPost.reaction_counts;
    ...
    // Handle reactions object format
      ...
      rawPost.reactions &&
      typeof rawPost.reactions === "object" &&
      !Array.isArray(rawPost.reactions)
    ) {
      Object.entries(rawPost.reactions).forEach(
    ...
    const normalized = {
      ...
      authorName:
        ...
        (isCurrentUserPost
          ...
          : "Employee User"),
      authorAvatar:
        rawPost.author?.avatar ||
        rawPost.authorAvatar ||
        rawPost.author_avatar ||
        rawPost.avatar ||
      ...
      authorPosition:
        ...
        rawPost.position ||
        (isCurrentUserPost
          ? user?.position || user?.job_title || "Employee"
          : user?.position || user?.job_title || "Employee"),
      authorEmail:
        rawPost.author?.email || rawPost.authorEmail || rawPost.author_email,
      ...
      images: (() => {
        ...
        const mediaImages = (rawPost.media || [])
          .filter((item) => {
            ...
            const isImageByExtension = normalizedUrl.match(
              /\.(jpeg|jpg|gif|png|webp|bmp|svg)$/i
            );
            const isImageByType = item.type === "image";
            const isImageByMimeType =
            ...
            const likelyImage =
              !item.type && (isImageByExtension || hasImagePattern);
        ...
        const uniqueImages = allImages.filter(
          (image, index, self) =>
            image &&
            image.url &&
            index === self.findIndex((img) => img && img.url === image.url)
        );
      ...
      videos: (() => {
        ...
        const mediaVideos = (rawPost.media || [])
          .filter((item) => {
            if (!item || !item.link) return false;
        ...
        const uniqueVideos = allVideos.filter(
          (video, index, self) =>
            video &&
            video.url &&
            index === self.findIndex((vid) => vid && vid.url === video.url)
        );
      ...
      documents: (() => {
        ...
        const mediaDocuments = (rawPost.media || [])
          .filter((item) => {
            if (!item || !item.link) return false;
            ...
            return (
              isDocumentByExtension || isDocumentByType || isDocumentByMimeType
            );
          })
          .map((item) => {
            let actualUrl = item.link;
        ...
        const uniqueDocuments = allDocuments.filter(
          (doc, index, self) =>
            doc &&
            doc.url &&
            index === self.findIndex((d) => d && d.url === doc.url)
        );
      ...
      links: (() => {
        ...
        const mediaLinks = (rawPost.media || [])
          .filter((item) => {
            if (!item || !item.link) return false;
            ...
            const isImage =
              decodedUrl.match(/\.(jpeg|jpg|gif|png|webp|bmp|svg)$/i) ||
              item.type === "image" ||
              (item.mime_type && item.mime_type.startsWith("image/"));
            const isVideo =
              decodedUrl.match(/\.(mp4|avi|mov|wmv|flv|mkv|webm)$/i) ||
              item.type === "video" ||
              (item.mime_type && item.mime_type.startsWith("video/"));
            const isDocument =
              decodedUrl.match(
        ...
        const uniqueLinks = allLinks.filter(
          (link, index, self) =>
            link &&
            link.url &&
            index === self.findIndex((l) => l && l.url === link.url)
        );
  ...
  const getPostId = () => {
    const postId =
      normalizedPost.post_id || normalizedPost.id || post.post_id || post.id;
    return postId;
  };
  ...
  const checkIsLiked = () => {
    ...
    const hasUserReactionFromBackend =
      normalizedPost.user_reaction !== null &&
      normalizedPost.user_reaction !== undefined;

    // Check local user reaction tracking (most reliable for current session)
    const hasReactedLocally = hasUserReacted
      ? hasUserReacted(postId, "love")
      : false;
    ...
    if (normalizedPost.reactions) {
      for (const [reactionType, reaction] of Object.entries(
        normalizedPost.reactions
      )) {
        const isInReactionUsers =
    ...
    // Only show heart as filled if current user actually reacted
    // Do NOT use postHasReactions as fallback—that would mark it liked for other users' reactions
    const liked =
      hasUserReactionFromBackend ||
      hasReactedLocally ||
  ...
  const handleLike = (event) => {
    ...
    // If user has ANY reaction, remove it (toggle off)
    // If user has NO reaction, add love reaction
    const userReaction = getUserReaction();

    if (userReaction) {
      // User has a reaction, so remove it
      addReaction(postId, userReaction, null, activeView, () => userReaction);
    } else {
      // No reaction, so add love
      addReaction(postId, "love", "❤️", activeView, () => null);
    }
  };
  ...
  const handleReaction = (reactionType, event) => {
    ...
    const emoji =
      emojiReactions.find((r) => r.name === reactionType)?.emoji || "👍";
  ...
  const getUserReaction = () => {
    ...
    // This is set by the backend and persists through refreshes
    if (
      normalizedPost.current_user_reaction !== null &&
      normalizedPost.current_user_reaction !== undefined
    ) {
  ...
  const getCommentUserReaction = (comment) => {
    ...
    for (const [reactionType, reaction] of Object.entries(comment.reactions)) {
      ...
      const hasReactedLocally = hasUserReactedToComment
        ? hasUserReactedToComment(commentId, reactionType)
        : false;
      ...
      if (Array.isArray(reaction)) {
        ...
        hasReaction = reaction.some(
          (r) =>
            currentUserIds.includes(r.user_id) || currentUserIds.includes(r.id)
        );
  ...
  const getTotalReactions = () => {
    ...
    return Object.entries(normalizedPost.reactions)
      .filter(
        ([reactionType]) => reactionType !== "like" && reactionType !== "love"
      )
    ...
    return Object.entries(normalizedPost.reactions)
      ...
      .map(([type, reaction]) => ({
        ...
        emoji:
          type === "like" || type === "love"
            ? "❤️"
            : emojiReactions.find((r) => r.name === type)?.emoji || "👍",
        count: reaction.count,
      }));
  };
