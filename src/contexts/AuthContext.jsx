...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (username, email) => {
    try {
      console.log('🚀 Starting frontend login for:', { username, email });

      // Check registered users first
      const existingUser = findUser(username, email);
      if (existingUser) {
        const userSession = storeUserLocally(existingUser);
        console.log('✅ Login successful with registered user:', userSession);
        return { success: true, user: userSession };
      }
      ...
      if ((username === 'user123' && email === 'user@gmail.com') || 
        ...
        localStorage.setItem('hrUser', JSON.stringify(userSession));
        setUser(userSession);
        console.log('✅ Login successful with demo user:', userSession);
        return { success: true, user: userSession };
      }
