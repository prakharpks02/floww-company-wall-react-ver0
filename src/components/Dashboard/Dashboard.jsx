...
const Dashboard = () => {
  ...
  const chatContent =
    isChatOpen && !isChatMinimized && !isCompactMode && !isFullScreenMobile ? (
      <ChatApp
  ...
  const renderMainContent = () => {
    ...
    if (activeView === "myposts") {
      return (
        <MyPosts filters={filters} onPostsChange={handleUserPostsChange} />
      );
    }
      ...
      <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 lg:p-6 border border-gray-200">
          ...
          <p className="text-sm lg:text-base text-gray-600">
            Share updates, collaborate with colleagues, and stay connected with
            the HR team.
          </p>
          {user?.is_admin && (
            <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-sm text-amber-800">
                🛡️ <strong>Admin Access:</strong> You have administrative
                privileges. Use the admin panel in the sidebar to manage posts,
                users, and reports.
              </p>
            </div>
          )}
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-sm text-red-800 mt-1">
                  Your account has been restricted. You can view and share posts
                  but cannot create new posts or add comments.
                </p>
              </div>
            </div>
          </div>
