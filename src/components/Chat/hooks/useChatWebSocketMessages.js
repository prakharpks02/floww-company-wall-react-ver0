...
export const useChatWebSocketMessages = ({
  ...
  const handleWebSocketMessage = useCallback(
    (messageData) => {
      ...
      const messageExists = existingMessages.some(
        (msg) => msg.id === messageData.message_id
      );
      ...
      const optimisticMessage = existingMessages.find((msg) => {
        ...
        const hasFileUrls =
          (messageData.file_urls && messageData.file_urls.length > 0) ||
          ...
          if (incomingUrls.length > 0 && optimisticUrls.length > 0) {
            fileUrlsMatch = incomingUrls.some((url) =>
              optimisticUrls.some(
                (optUrl) =>
                  url === optUrl ||
                  url.includes(optUrl.split("/").pop()) ||
                  optUrl.includes(url.split("/").pop())
              )
            );
          }
        }
        ...
        return (
          isOptimistic && senderMatches && contentMatches && timeWithinWindow
        );
      });
          ...
          const updated = prev[conversationKey].map((msg) => {
            if (msg.id === optimisticMessage.id) {
              return {
                ...
                timestamp: new Date(
                  messageData.created_at || messageData.timestamp || Date.now()
                ),
      ...
      // Create message object - PRESERVE SENDER DATA and normalize names
      const incomingMessage = {
        id: messageData.message_id || `ws-${Date.now()}`,
        senderId: senderEmployeeId,
        // Preserve raw sender but also provide normalized fields for UI
      ...
      if (messageData.reply_to_message_id || messageData.reply_to_message) {
        ...
        incomingMessage.replyTo = {
          ...
          text:
            replyData.content || replyData.reply_content || "Original message",
          senderId: replyData.sender?.employee_id || replyData.reply_sender_id,
          senderName:
            replyData.sender?.employee_name ||
            replyData.reply_sender_name ||
            "Unknown",
    ...
    [activeConversation, messages, setMessages, currentUser]
      ...
      if (response.status === "success" && Array.isArray(response.data)) {
        const formattedMessages = response.data.map((msg) => {
          return {
            ...
            sender: msg.sender || null, // raw sender
            senderName:
            ...
            ...(msg.reply_to_message_id && {
              replyTo: {
                ...
                senderName:
                  msg.reply_to_message?.sender?.employee_name ||
  ...
  useEffect(() => {
    const unsubscribeMessage = enhancedChatAPI.onMessage(
      handleWebSocketMessage
    );
    const unsubscribeConnection = enhancedChatAPI.onConnection(
