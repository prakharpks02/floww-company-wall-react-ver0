...
export const useChatMessageHandlers = ({
  ...
  const handleSendMessage = async () => {
    ...
    const hasContent =
      newMessage.trim() || (pendingFileUrls && pendingFileUrls.length > 0);
    ...
    if (!activeConversation.room_id) {
      ...
      if (activeConversation.type === "direct") {
        ...
        if (otherParticipantId) {
          try {
            let roomResponse = await enhancedChatAPI.findRoomWithParticipant(
              String(otherParticipantId)
            );

            if (!roomResponse || !roomResponse.room_id) {
              roomResponse = await enhancedChatAPI.createRoomAndConnect(
                String(otherParticipantId)
              );
            } else {
    ...
    if (
      ...
      connectionStatus.roomId !== activeConversation.room_id
    ) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (
          status.isConnected &&
          status.roomId === activeConversation.room_id
        ) {
          connected = true;
          break;
        }
      ...
      if (!connected) {
        console.error(
          "[Message] Failed to establish WebSocket connection after waiting"
        );
        chatToast.error(
          "Could not connect to chat. Please refresh and try again."
        );
        return;
      }
    }
        ...
        const messageData = {
          ...
          sender: {
            ...
            profile_picture_link:
              currentUser?.profile_picture_link || currentUser?.avatar || "",
          },
          text: messageText,
        ...
        setConversations((prev) =>
          prev.map((conv) =>
            conv.id === activeConversation.id
              ? {
                  ...conv,
                  lastMessage: { text: messageText, timestamp: new Date() },
                }
              : conv
          )
        );
  ...
  const handleIncomingMessage = (messageData) => {
    ...
    const senderEmployeeId =
      messageData.sender?.employee_id || messageData.sender_id;
    ...
    const incomingMessage = {
      ...
      timestamp: new Date(
        messageData.timestamp || messageData.created_at || Date.now()
      ),
      read: false,
      status: "received",
    };

    // Attach sender object to avoid 'Unknown' rendering in UI
    incomingMessage.sender = messageData.sender
      ? {
          id: messageData.sender.employee_id,
          name:
            messageData.sender.employee_name ||
            messageData.sender.employee_username ||
            "Unknown",
          username: messageData.sender.employee_username || null,
          avatar: messageData.sender.profile_picture_link || null,
          jobTitle: messageData.sender.job_title || null,
        }
      : null;
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const result = await enhancedChatAPI.editMessage(
        editingMessage.id,
      ...
      if (result.status === "success") {
        ...
        setMessages((prev) => ({
          ...prev,
          [activeConversation.id]: (prev[activeConversation.id] || []).map(
            (msg) =>
  ...
  const loadPreviousMessages = async (roomId, conversationId) => {
    try {
      ...
      if (messagesResponse.status === "success" && messagesResponse.data) {
        const messagesWithReplies = messagesResponse.data.filter(
      ...
      if (messagesResponse.status === "success" && messagesResponse.data) {
        ...
        const messagesWithReplies = convertedMessages.filter(
          (msg) => msg.replyTo
        );
    ...
    if (!conversation.room_id && conversation.type === "direct") {
      try {
        ...
        if (otherParticipantId) {
          ...
          const roomResponse = await enhancedChatAPI.findRoomWithParticipant(
            String(otherParticipantId)
          );
          ...
          if (roomResponse && roomResponse.room_id) {
            ...
            const newRoomResponse = await enhancedChatAPI.createRoomAndConnect(
              String(otherParticipantId)
            );
  ...
  const handleStartEdit = (message) => {
    if (
      ...
      message.senderId !== "emp-" + currentUser?.id
    ) {
      return;
    }
  ...
  const handleForwardMessage = async (conversationIds, forwardMessage) => {
    ...
    try {
      ...
      for (const conversationId of conversationIds) {
        ...
        if (targetConversation) {
          ...
          if (!roomId && targetConversation.type === "direct") {
            const otherParticipantId = targetConversation.participants.find(
              (id) => id !== currentUser?.id
            );
      ...
      if (result && result.status === "success") {
        chatToast.success(
          `Message forwarded to ${roomIds.length} conversation${
            roomIds.length > 1 ? "s" : ""
          }`
        );
        ...
        if (
          activeConversation &&
          roomIds.includes(activeConversation.room_id)
        ) {
          try {
